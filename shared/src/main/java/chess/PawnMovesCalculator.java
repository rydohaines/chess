...
public class PawnMovesCalculator implements PieceMovesCalculator{
    ...
    public Collection<ChessMove> pieceMoves(ChessBoard board, ChessPosition position) {
        if(board.getPiece(position).getTeamColor() == WHITE) {
            ...
            if(position.getRow() == 2 && board.getPiece(position.upOne().upOne()) == null && position.upOne() != position.upOne().upOne()){
            ...
            if(position.getRow() == 7 && board.getPiece(position.downOne().downOne()) == null && position.downOne() != position.downOne().upOne()){
